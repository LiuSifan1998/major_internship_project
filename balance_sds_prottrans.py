# train classifiers on down-size sampling processed class-balanced dataset using representations from bert, xl, xlnet
import pandas as pd
import numpy as np
...
# bert train set /projects/qbio/bifo/ProtProduction/khld071/protransbert_output/cdhit_bert.csv
# bert test set /projects/qbio/bifo/ProtProduction/khld071/protransbert_output/cdhit_bert.csv
# xl train set /projects/qbio/bifo/ProtProduction/khld071/protransxl_output/cdhit_xl.csv
# xl test set /projects/qbio/bifo/ProtProduction/khld071/protransxl_output/cdhit_xl.csv
# xlnet train set /projects/qbio/bifo/ProtProduction/khld071/protransxlnet_output/cdhit_xlnet.csv
# xlnet test set /projects/qbio/bifo/ProtProduction/khld071/protransxlnet_output/cdhit_xlnet.csv
...
def type_transformer(transformer_name,classifier_name,file_train,file_test):
    ...
    if transformer_name == 'bert':
        output_file = '/projects/qbio/bifo/ProtProduction/khld071/protransbert_output/final_model/score/score4final' + classifier_name + '_balance_sds.csv'
    if transformer_name == 'xl':
        output_file = '/projects/qbio/bifo/ProtProduction/khld071/protransxl_output/final_model/score/score4final' + classifier_name + '_balance_sds.csv'
    if transformer_name == 'xlnet':
        output_file = '/projects/qbio/bifo/ProtProduction/khld071/protransxlnet_output/final_model/score/score4final' + classifier_name + '_balance_sds.csv'
...
df_0 = df[df.symbol == 0].sample(frac=1).reset_index(drop=True)
fraction = len(df_0) / (len(df) - len(df_0))
df_1 = df[df.symbol == 1].sample(frac=fraction).reset_index(drop=True)
sub_df = df_1.append(df_0).sample(frac=1.0).reset_index(drop=True)
X = sub_df.drop(['symbol'], axis = 1)
Y = sub_df.iloc[:,-1]
X,Y = X.to_numpy(),Y.tolist()
id_df = pd.read_csv(transformer_train,low_memory=False)
